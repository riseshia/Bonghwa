(function ($) {
  ...
  app.FirewoodView = Backbone.View.extend({
    ...
    delete: function (e) {
      e.stopPropagation()
      const self = this
      const dataId = this.model.get("id")
    ...
    unFold: function () {
      ...
      const fws = app.firewoods.getPreviousFws(this.model, 5)
      const view = this
      if ( this.model.get("prev_mt") !== 0 && fws.length == 0 ) {
        ...
        this.model.ajaxMtLoad().then((json) => {
    ...
    fold: function () {
      ...
      this.$(".fw-sub").slideUp(() => { subView.remove() })
